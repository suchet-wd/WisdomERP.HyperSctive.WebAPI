                ...
                if (Cnn.ExecuteOnly(_Qry, WSM.Conn.DB.DataBaseName.DB_VENDER))
                {
                    ...
                    if (dtPO != null)
                    {
                        ...
                        if (Cnn.ExecuteOnly(_Qry, WSM.Conn.DB.DataBaseName.DB_VENDER))
                        {
                            ...
                            _Qry += " SELECT ISNULL(FTDataKey, '') AS FTDataKey, ISNULL(FTDateCreate, '') AS FTDateCreate,ISNULL(VenderCode, '')," +
                                "ISNULL(VendorName, '') AS VendorName, ISNULL(VendorLocation, '') AS VendorLocation, ISNULL(FactoryCode, '') AS FactoryCode," +
                                "ISNULL(PONo, '') AS PONo,ISNULL(PODate, '') AS PODate, ISNULL(Shipto, '') AS Shipto, ISNULL(GarmentShipmentDestination, '') AS GarmentShipmentDestination," +
                                ...
                                "ISNULL(CareCode, '') AS CareCode, ISNULL(Color, '') AS Color, ISNULL(ColorName, '') AS ColorName, ISNULL(GCW, '') AS GCW, ISNULL(Size, '') AS Size, " +
                                "ISNULL(SizeMatrix, '') AS SizeMatrix, ISNULL(Currency, '') AS Currency, ISNULL(Price, 0) AS Price, ISNULL(Quantity, 0) AS Quantity," +
                                "ISNULL(QtyUnit, '') AS QtyUnit, ISNULL(DeliveryDate, '') AS DeliveryDate, ISNULL(Season, '') AS Season, ISNULL(Custporef, ''),ISNULL(Buy, ''),ISNULL(BuyNo, '')," +
                                "ISNULL(Category, '') AS Category, ISNULL(Program, '') AS Program, ISNULL(SubProgram, '') AS SubProgram, ISNULL(StyleNo, '')," +
                                "ISNULL(StyleName, ''),ISNULL(POMatching1, ''), ISNULL(POMatching2, ''),ISNULL(POMatching3, '') AS POMatching3, ISNULL(POMatching4, ''),ISNULL(POMatching5, '') AS POMatching5, ISNULL(ItemMatching1, '') AS ItemMatching1," +
                                "ISNULL(ItemMatching2, '') AS ItemMatching2,ISNULL(ItemMatching3, '') AS ItemMatching3, ISNULL(ItemMatching4, '') AS ItemMatching4, ISNULL(ItemMatching5, '') AS ItemMatching5, ISNULL(Position, '') AS Position," +
                                "ISNULL(Type, '') AS Type, ISNULL(PriceTerm, ''),ISNULL(PaymentTerm, ''),ISNULL(Remarkfrommer, '') AS Remarkfrommer, ISNULL(RemarkForPurchase, '') AS RemarkForPurchase," +
                                "ISNULL(InvoiceCmpCode, '') AS InvoiceCmpCode, ISNULL(CompanyName, '') AS CompanyName, ISNULL(address1, '') AS address1, " +
                                "ISNULL(address2, '') AS address2, ISNULL(address3, '') AS address3, ISNULL(address4, '') AS address4, " +
                                "ISNULL(sysowner, '') AS sysowner, ISNULL(ZeroInspection, '') AS ZeroInspection, ISNULL(GarmentShip, '') AS GarmentShip, " +
                                "ISNULL(OGACDate, '') AS OGACDate, ISNULL(HITLink, '') AS HITLink, ISNULL(NIKECustomerPONo, '') AS NIKECustomerPONo, " +
                                "ISNULL(QRS, 0) AS QRS, ISNULL(PromoQty, 0) AS PromoQty, ISNULL(ActualQuantity, 0) AS ActualQuantity, ISNULL(POUploadDate, '') AS POUploadDate," +
                                "ISNULL(POUploadTime, '') AS POUploadTime, ISNULL(POUploadBy, '') AS POUploadBy, ISNULL(CountryOfOrigin, '') AS CountryOfOrigin, ISNULL(SaleOrderSaleOrderLine, '') AS SaleOrderSaleOrderLine," +
                                "ISNULL(NikeSAPPOPONIKEPOLine, '') AS NikeSAPPOPONIKEPOLine, ISNULL(NikematerialStyleColorway, '') AS NikematerialStyleColorway, ISNULL(Modifire1, '') AS Modifire1,ISNULL(Modifire2, '') AS Modifire2," +
                                "ISNULL(Modifire3, '') AS Modifire3, ISNULL(MPOHZ, '') AS MPOHZ, ISNULL(ItemDescription, '') AS ItemDescription, ISNULL(BulkQRSSample, '') AS BulkQRSSample, ISNULL(ExtraMinQty, 0) AS ExtraMinQty," +
                                "ISNULL(EAGSystemUnitPriceERP3, 0) AS EAGSystemUnitPriceERP3, ISNULL(CCTotalpage, 0) AS CCTotalpage, ISNULL(Surcharge, 0) AS Surcharge,ISNULL(POApproveDate, '') AS POApproveDate," +
                                "ISNULL(POIssueDate, '') AS POIssueDate,ISNULL(CLXorderconfirmationnumber, '') AS CLXorderconfirmationnumber, ISNULL(FTYMerch, '') AS FTYMerch,ISNULL(HKMerch, '') AS HKMerch, ISNULL(ChinaInsertCard, '') AS ChinaInsertCard," +
                                "ISNULL(P1pc2in1, '') AS P1pc2in1,ISNULL(ReplyCode, '') AS ReplyCode, ISNULL(WovenLabelSizeLength, '') AS WovenLabelSizeLength,ISNULL(ArgentinaImportNumber, '') AS ArgentinaImportNumber,ISNULL(DownFill, '') AS DownFill," +
                                "ISNULL(SYS_ID, '') AS SYS_ID, ISNULL(ChinasizeMatrixtype, '') AS ChinasizeMatrixtype, ISNULL(EAGERPItemNumber, '') AS EAGERPItemNumber,ISNULL(CompoundColororCCIMfor2in1CCIM, '') AS CompoundColororCCIMfor2in1CCIM," +
                                "ISNULL(CPO, '') AS CPO,ISNULL(BhasaIndonesiaProductBrand, '') AS BhasaIndonesiaProductBrand, ISNULL(SAFCODE, '') AS SAFCODE, ISNULL(Youthorder, '') AS Youthorder, ISNULL(NFCproduct, '') AS NFCproduct, " +
                                "ISNULL(NeckneckopeningX2, '') AS NeckneckopeningX2, ISNULL(ChestbodywidthX2, '') AS ChestbodywidthX2, ISNULL(CenterBackbodylength, '') AS CenterBackbodylength, ISNULL(WaistwaistrelaxedInseam, '') AS WaistwaistrelaxedInseam," +
                                "ISNULL(PackQuantityQTYPERSELLINGUNIT, 0) AS PackQuantityQTYPERSELLINGUNIT, ISNULL(Fabriccode, '') AS Fabriccode, ISNULL(PRODUCTDESCRIPTION, '') AS PRODUCTDESCRIPTION, ISNULL(PRODUCTCODEDESCRIPTION, '') AS PRODUCTCODEDESCRIPTION," +
                                "ISNULL(GARMENTSTYLEFIELD, '') AS GARMENTSTYLEFIELD, ISNULL(INSEAMSTYLE, '') AS INSEAMSTYLE FROM [" + WSM.Conn.DB.DataBaseName.DB_VENDER + "].dbo.POToVender AS POV " +
                                "WHERE POV.vendercode = '" + vencodeRq + "'"; // AND POV.StateAcknowledge = 0";
