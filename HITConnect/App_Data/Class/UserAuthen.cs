...
namespace HyperConvert
{
    public class UserAuthen
    {
        ...
        public static DataTable GetDTUserValidate(WSM.Conn.SQLConn Cnn, UserAuthen value)
        {
            ...
            if (value.id != "" &&  value.token == "") 
            {
                _Qry = " SELECT V.Pwd AS pwd, V.VenderMailLogIn AS VenderMailLogIn , VUP.VenderGrp AS VenderGrp " +
                    ...
                    " LEFT JOIN [" + WSM.Conn.DB.DataBaseName.HITECH_MERCHAN + "].dbo.AuthenKeys AS ATK WITH ( NOLOCK ) " +
                    " ON ATK.VenderMailLogIn = V.VenderMailLogIn  " +
                    " LEFT JOIN [" + WSM.Conn.DB.DataBaseName.HITECH_MERCHAN + "].dbo.VenderUserPermissionCmp VUP WITH ( NOLOCK ) " +
            ...
            else if (value.id != "" &&  value.token != "") 
            {
                _Qry = " SELECT V.Pwd AS pwd, V.VenderMailLogIn AS VenderMailLogIn , VUP.VenderGrp AS VenderGrp " +
                    ...
                    " LEFT JOIN [" + WSM.Conn.DB.DataBaseName.HITECH_MERCHAN + "].dbo.AuthenKeys AS ATK WITH ( NOLOCK ) " +
                    " ON ATK.VenderMailLogIn = V.VenderMailLogIn  " +
                    " LEFT JOIN [" + WSM.Conn.DB.DataBaseName.HITECH_MERCHAN + "].dbo.VenderUserPermissionCmp AS VUP WITH ( NOLOCK ) " +
                    " ON V.VenderMailLogIn = VUP.VenderMailLogIn " +
        ...
        public static bool DelAuthenKey(WSM.Conn.SQLConn Cnn, string value)
        {
            ...
            try
            {
                _Qry = " DELETE FROM [" + WSM.Conn.DB.DataBaseName.HITECH_MERCHAN + "].dbo.AuthenKeys WHERE VenderMailLogIn = '" + value + "'";
